from dataclasses import dataclass
from collections import UserDict
from types import MappingProxyType
from toolz import dicttoolz, functoolz, itertoolz


@dataclass
class DataDict(UserDict):
    def __init__(self, data):
        self.data = MappingProxyType(data)

    def pipe(self, *funcs):
        return DataDict(functoolz.pipe(self.data, *funcs))

    def items(self):
        return self.data.items()

    def values(self):
        return self.data.values()

    def keys(self):
        return self.data.keys()


@dataclass
class Enhanced_DataDict(UserDict):
    def __init__(self, data):
        self.data = MappingProxyType(data)

    def pipe(self, *funcs):
        return Enhanced_DataDict(pipe(self.data, *funcs))

    # same as native get except can accept list of indices
    def iget(self, ind, default="__no__default__"):
        return itertoolz.get(ind, self.data, default)

    def itemmap(self, func):
        return Enhanced_DataDict(dicttoolz.itemmap(func, self.data))

    def keymap(self, func):
        return Enhanced_DataDict(dicttoolz.keymap(func, self.data))

    def valmap(self, func):
        return Enhanced_DataDict(dicttoolz.valmap(func, self.data))

    def itemfilter(self, predicate):
        return Enhanced_DataDict(dicttoolz.itemfilter(predicate, self.data))

    def keyfilter(self, predicate):
        return Enhanced_DataDict(dicttoolz.keyfilter(predicate, self.data))

    def valfilter(self, predicate):
        return Enhanced_DataDict(dicttoolz.valfilter(predicate, self.data))

    def merge(self, *dicts, **kwargs):
        return Enhanced_DataDict(dicttoolz.merge(self.data, *dicts, **kwargs))

    def merge_with(self, func, *dicts, **kwargs):
        return Enhanced_DataDict(
            dicttoolz.merge_with(func, self.data, *dicts, **kwargs)
        )

    def update_in(self, keys, func, default=None):
        return Enhanced_DataDict(dicttoolz.update_in(self.data, keys, func, default))

    def assoc(self, key, value):
        return Enhanced_DataDict(dicttoolz.assoc(self.data, key, value))

    def assoc_in(self, keys, value):
        return Enhanced_DataDict(dicttoolz.assoc_in(self.data, keys, value))

    def dissoc(self, *keys, **kwargs):
        return Enhanced_DataDict(dicttoolz.dissoc(self.data, *keys, **kwargs))

    def pick(self, seq):
        return Enhanced_DataDict(dicttoolz.keyfilter(lambda k: k in seq, self.data))

    def items(self):
        return self.data.items()

    def values(self):
        return self.data.values()

    def keys(self):
        return self.data.keys()


@dataclass
class Chain_DataDict(UserDict):
    def __init__(self, data):
        self.data = data

    def pipe(self, *funcs):
        return Chain_DataDict(pipe(self.data, *funcs))

    # same as native get except can accept list of indices
    def iget(self, ind, default="__no__default__"):
        return Chain_DataDict(itertoolz.get(ind, self.data, default))

    def itemmap(self, func):
        return Chain_DataDict(dicttoolz.itemmap(func, self.data))

    def keymap(self, func):
        return Chain_DataDict(dicttoolz.keymap(func, self.data))

    def valmap(self, func):
        return Chain_DataDict(dicttoolz.valmap(func, self.data))

    def itemfilter(self, predicate):
        return Chain_DataDict(dicttoolz.itemfilter(predicate, self.data))

    def keyfilter(self, predicate):
        return Chain_DataDict(dicttoolz.keyfilter(predicate, self.data))

    def valfilter(self, predicate):
        return Chain_DataDict(dicttoolz.valfilter(predicate, self.data))

    def merge(self, *dicts, **kwargs):
        return Chain_DataDict(dicttoolz.merge(self.data, *dicts, **kwargs))

    def merge_with(self, func, *dicts, **kwargs):
        return Chain_DataDict(dicttoolz.merge_with(func, self.data, *dicts, **kwargs))

    def update_in(self, keys, func, default=None):
        return Chain_DataDict(dicttoolz.update_in(self.data, keys, func, default))

    def assoc(self, key, value):
        return Chain_DataDict(dicttoolz.assoc(self.data, key, value))

    def assoc_in(self, keys, value):
        return Chain_DataDict(dicttoolz.assoc_in(self.data, keys, value))

    def dissoc(self, *keys, **kwargs):
        return Chain_DataDict(dicttoolz.dissoc(self.data, *keys, **kwargs))

    def pick(self, seq):
        return Chain_DataDict(dicttoolz.keyfilter(lambda k: k in seq, self.data))

    def clear(self):
        return {}

    def pop(self, key):
        return self.data.copy().pop(key)

    def update(self, other):
        return self.data.copy().update(other)
